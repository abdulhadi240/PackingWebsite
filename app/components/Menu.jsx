...
const Menu = ({border , background , color , hover , margin , active,hide}) => {
  return (
    <div className={`h-10   py-auto sm:w-full ${background ? 'bg-[#98ca3e]' : ''}  mt-${margin} lg:block ${hide?'hidden' : 'block'}`}>
        <div className={` -ml-8 grid grid-cols-3  text-[10px] sm:text-base sm:w-full sm:flex sm:justify-between text-center   ${color?`text-white` : 'text-gray-400'} overflow-hidden`}>
